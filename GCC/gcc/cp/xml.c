...
xml_output_function_decl (xml_dump_info_p xdi, tree fd, xml_dump_node_p dn)
{
  ...
  if ((arg_type == void_list_node) && (!(body && 1)))
    {
    fprintf (xdi->file, "/>\n");
    return;
    }
  ...
  /* <Modification by="Daniel J. Lauk"> */
  if (1 && body) do_xml_dump_body(xdi, body, 2);
  /* </Modification> */
...
xml_output_var_decl (xml_dump_info_p xdi, tree vd, xml_dump_node_p dn)
{
  ...
  if(DECL_INITIAL(vd) == NULL_TREE){
    fprintf (xdi->file, " />\n");
  }else{ 
    fprintf (xdi->file, ">\n");
    do_xml_dump_expr(xdi, DECL_INITIAL(vd), 2);
    fprintf (xdi->file, "  </Variable>\n");
  }
}
